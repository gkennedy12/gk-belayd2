...
#include "../plugins/utils/pressure_utils.h"
	       ...
	       const struct belayd_cause * const cse)
{
	...
	ret = belayd_parse_int(args_obj, "threshold", &opts->threshold);
        ...
        ret = belayd_parse_string(args_obj, "qmp_path", &qmp_path_str);
        if (ret)
                goto error;
	...
	ret = belayd_parse_string(args_obj, "managed_str", &managed_str);
	if (!ret) {
	...
	ret = belayd_parse_int(args_obj, "post_action_delay", &opts->post_action_delay);
	if (verbose) fprintf(stderr, "mem_resize_init: opts->post_action_delay: %d\n",
		opts->post_action_delay);

	opts->memory_reclaim = 0;
	ret = belayd_parse_int(args_obj, "memory_reclaim", &opts->memory_reclaim);
	if (verbose) fprintf(stderr, "mem_resize_init: opts->memory_reclaim: %d\n",
		opts->memory_reclaim);

	opts->max_mem = 0;
	ret = belayd_parse_int(args_obj, "max_mem", &max_mem_int);
	if (ret == 0)
		opts->max_mem = (long long)max_mem_int * GB;
	if (verbose) fprintf(stderr, "mem_resize_init: %dGB, opts->max_mem: %lld\n",
		max_mem_int, opts->max_mem);
